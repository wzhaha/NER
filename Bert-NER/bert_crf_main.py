...
def train(config, train_dataset, model, tokenizer):
    ...
    config.warmup_steps = int(t_total * 0.1)
    ...
    for epoch_num in range(int(config.epochs)):
        ...
        if config.evaluate_during_training:
            ...
            output_dir = os.path.join(config.save, "bert_crf_checkpoint-{}".format(epoch_num))
...
def evaluate(config, model, tokenizer, prefix=""):
    ...
    for batch in eval_dataloader:
        ...
        nb_eval_steps += 1
        if preds is None:
            preds = tags.squeeze(0).detach().cpu().numpy()
            out_label_ids = inputs["labels"].detach().cpu().numpy()
            masks = inputs["attention_mask"].detach().cpu().numpy()
        else:
            preds = np.append(preds, tags.squeeze(0).detach().cpu().numpy(), axis=0)
